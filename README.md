...
演示数据存在浏览器 localStorage，键是 `yolink-demo-v5`（`src/store/store.ts` 的 `STORAGE_KEY`）；模型改了就把版本号加一，老数据自动作废、重新灌种子。
...
| 客服工作台 | `/workbench` | 顾问 林薇，以「林顾问」身份。按 PC 客户端布局：左侧图标栏导航（按员工能力与模块显隐：会话、客户、群发、邀请链接、群活跃助手、提现审核、设置）、三栏可拖宽、右栏可收起；话术库（右栏「话术」页签 + 打字自动匹配 + 图片 / 文件直发）；右下角浮钮是演示控制（切换登录员工、打开手机屏 / 后台） |
...
src/
  domain/
    ...
    seed-quick-replies.ts 话术库种子：演示附件（public/media 下的费率表、流程图、说明书 PDF 等）、企业 / 个人分类、18 条话术（含图片、文件、停用）
  ...
  apps/
    ...
    workbench/    工作台（PC 客户端布局）：components/layout 图标栏 / 坐席切换 / 拖拽把手 / 演示浮条 / 系统通知，
                  ...
                  components/quick-replies 右栏「话术」页签、打字自动匹配浮层、`/` 列表，
