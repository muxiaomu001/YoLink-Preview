...
1. 工作台消息旁点击 `···` 或右键，打开图标配文字的竖向分组菜单，可回复、编辑、拷贝文本、转发和撤回；群置顶与管理员删除按权限显示。
2. 私聊发送后显示未读；右下角「演示控制」打开当前客户会话，客户看到消息后工作台变为已读。群消息可点已读人数查看名单，仅工作台提供。
3. 编辑后两端同步显示“已编辑”；坐席撤回后客户侧直接移除，预览与引用也不残留；后台消息审计可查看原文和修改历史。
4. 后台「策略与能力开关 → 数值型策略」按坐席／客户分别配置编辑和撤回，支持分钟、小时、天及不限时间。为方便体验初值不限；正式默认值待确认。
5. 群里使用 `@` 选择成员后发送；消息里的名字可点击，客户查看资料仍受策略限制。表情按钮可实际插入并发送常用表情。
6. 人像来自本地虚构人物图集 `public/media/avatars/people-grid.png`，无第三方头像请求。官方身份不会随着员工交接换头像。

边界：仍未完成语音／视频消息、发送中／失败重试、上传进度、正在输入、跨会话草稿等完整体验。完整产品对照见本地 `../docs/prd/19-工作台完整体验检查.md`。本地演示不证明真实推送、网络同步或大群容量。
