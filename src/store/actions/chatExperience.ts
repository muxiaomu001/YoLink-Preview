...
export function chatExperienceActions(set: Set, get: Get): ChatExperienceActions {
  ...
  return {
    deleteChatMessages: (ids, actor, everyone) => {
      ...
      set({ messages: s.messages.map((m) => unique.includes(m.id) ? everyone ? { ...m, deletedAt: at } : { ...m, hiddenFor: [...new Set([...(m.hiddenFor ?? []), key])] } : m),
        chatGroups: everyone?s.chatGroups.map((g)=>({...g,pinnedMessageIds:g.pinnedMessageIds.filter((id)=>!unique.includes(id))})):s.chatGroups,
        groupLogs: everyone ? [...Array.from(new Set(messages.map((m)=>s.conversations.find((c)=>c.id===m!.convId)?.chatGroupId).filter((id):id is string=>!!id))).map((groupId)=>({id:newId('glog'),groupId,at,actorKind:actor.kind,actorId:actor.id,action:'delete_message',detail:`为所有人删除 ${unique.length} 条消息，原文保留审计`})),...s.groupLogs] : s.groupLogs,
