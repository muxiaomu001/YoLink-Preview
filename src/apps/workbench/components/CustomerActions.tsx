...
export function CustomerActions({ c, open, onClose }: { c: Customer; open: boolean; onClose: () => void }) {
  ...
  const resetBlocker = customerPasswordResetBlocker(s, c.id, staff.id)
  const moderationBlocker = customerModerationBlocker(s, c.id, staff.id)
  ...
  const reset = async () => {
    ...
    if (!ok) return
    const result = s.resetCustomerPassword(c.id, staff.id)
  ...
  const kick = async () => {
    ...
    if (!ok) return
    const error = s.forceLogoutCustomer(c.id, staff.id)
  ...
  const toggleBan = async () => {
    ...
    if (!ok) return
    const error = s.setCustomerBan(c.id, !banned, staff.id)
    if (error) return toast(error, 'warn')
    toast(banned ? '已解除封禁' : '已封禁，客户无法登录')
  }
  const mute = (kind: 'allGroups' | 'global', hours: number | null) => {
    const error = kind === 'allGroups' ? s.muteCustomerAllGroups(c.id, hours, staff.id) : s.muteCustomerGlobally(c.id, hours, staff.id)
    if (error) return toast(error, 'warn')
  ...
  const unmute = (kind: 'allGroups' | 'global') => {
    onClose()
    const error = kind === 'allGroups' ? s.muteCustomerAllGroups(c.id, 0, staff.id) : s.muteCustomerGlobally(c.id, 0, staff.id)
    if (error) return toast(error, 'warn')
    ...
    <>
      {open && (
        <PopoverPanel align="right" width={184} className="py-1">
          <MenuItem disabled={!!resetBlocker} title={resetBlocker ?? '生成一次性新密码'} onClick={() => void reset()}>
            <KeyRound size={14} /> 重置密码
          </MenuItem>
          <MenuItem disabled={!!moderationBlocker} title={moderationBlocker ?? '让该客户的所有设备退出登录'} onClick={() => void kick()}>
            <LogOut size={14} /> 强制下线
          </MenuItem>
          {allGroupsMuted ? (
            <MenuItem disabled={!!moderationBlocker} title={moderationBlocker ?? undefined} onClick={() => unmute('allGroups')}>
              <MicOff size={14} /> 解除群聊禁言
            </MenuItem>
          ) : (
            <MenuItem
              disabled={!!moderationBlocker} title={moderationBlocker ?? undefined}
              onClick={() => {
                onClose()
                setMuting('allGroups')
              }}
          ...
          {globallyMuted ? (
            <MenuItem disabled={!!moderationBlocker} title={moderationBlocker ?? undefined} onClick={() => unmute('global')}>
          ...
          <MenuItem disabled={!!moderationBlocker} title={moderationBlocker ?? undefined} danger={!banned} onClick={() => void toggleBan()}>
