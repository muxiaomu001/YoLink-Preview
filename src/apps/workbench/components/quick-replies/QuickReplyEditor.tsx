/**
 * 新建 / 编辑个人话术的弹窗：类型（文字 / 图片 / 文件，新建时选）、标题、分类（个人分类 + 新建分类）、关键词、附件（本机选文件）、正文 / 说明。
...
import { KIND_META, parseKeywords } from './shared'
...
export function QuickReplyEditor({ initial, onClose }: { initial?: QuickReply; onClose: () => void }) {
  ...
  const save = () => {
    ...
    const saved = s.saveQuickReply('personal', { id: initial?.id, categoryId, kind: form.kind, title: form.title, text: form.text, keywords: parseKeywords(form.keywords), media: isText ? undefined : form.media }, staff.id)
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          {form.categoryId === NEW_CATEGORY ? (
            ...
            <Field label="关键词" hint="逗号或空格分隔">
              <Input value={form.keywords} onChange={(e) => patch({ keywords: e.target.value })} placeholder="如：约, 时间, 电话" />
            </Field>
          )}
        </div>
        {form.categoryId === NEW_CATEGORY && (
          <Field label="关键词" hint="逗号或空格分隔，打字时按这些词匹配">
            <Input value={form.keywords} onChange={(e) => patch({ keywords: e.target.value })} placeholder="如：约, 时间, 电话" />
          </Field>
        )}
        {!isText && (
