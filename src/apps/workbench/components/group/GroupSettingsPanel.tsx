...
export function GroupSettingsPanel({ group: g, actor, perm, compact, officialEditable }: GroupPanelProps & { officialEditable: boolean }) {
  ...
  const canOfficial = officialEditable && canInfo

  const patch = (p: Parameters<typeof s.updateGroupSettings>[1], msg: string) => {
    const result = s.updateGroupSettings(g.id, p, actor)
  ...
  return (
    <Section title="群设置" compact={compact}>
      <div className="space-y-3">
        <Row label="全员禁言" desc={g.settings.allMuted ? '开启中：普通客户成员不能发言，管理员与坐席不受限' : '开启后仅管理员与坐席可发言'} disabledReason={canInfo ? undefined : '需要「修改群信息」权限'}>
          <Switch checked={g.settings.allMuted} disabled={!canInfo} onChange={(v) => patch({ allMuted: v }, v ? '已开启全员禁言，管理员不受限' : '已关闭全员禁言')} />
        </Row>
        <Row label="慢速模式" desc={`当前企业策略 ${s.policyNumbers.slowModeSeconds} 秒`} disabledReason={canInfo ? undefined : '需要「修改群信息」权限'}>
          <Select
        ...
        <Row label={<>官方群标记 {g.official && <Pill tone="amber">官方</Pill>}</>} desc="标记后客户不能退出这个群" disabledReason={canOfficial ? undefined : '需要自己的实操坐席及「管理所有群」权限或管理后台入口'}>
          <Switch checked={g.official} disabled={!canOfficial} onChange={(v) => void toggleOfficial(v)} />
        </Row>
      </div>
    </Section>
  )
