...
export function BulkGroupModal({ ids, onClose }: { ids: string[]; onClose: () => void }) {
  ...
  const apply = () => {
    ...
    toast(r.added ? `已把 ${r.added} 人拉入「${g.name}」${r.skipped.length ? `，跳过 ${r.skipped.length} 人（已在群、已满、被封禁或不满足头衔条件）` : ''}` : '没有人被拉入：都已在群、已满、被封禁或不满足头衔条件', r.added ? 'ok' : 'warn')
    onClose()
  }
  return (
    <Modal
...
/** 导出字段按 PRD：昵称、账号 ID、手机号、邮箱、标签、注册时间、最近活跃、主归属坐席 */
