 ...
 *
 * 分两套是因为同一个词在两边意思完全不同——坐席说「保本」是合规事故，客户问「保本」
 * 只是提问。共用一套词库，要么放过坐席，要么天天误伤客户，没有第三种结果。
 */
...
export function SensitiveWordsPage() {
  ...
  return (
    <div>
      ...
      {tab === 'seat' && (
        <Note className="mt-3">
          查的是<b>坐席发给客户的话</b>，拦的是合规风险：承诺收益、私下转账、绕开平台。坐席被拦下时提示里会写明是合规词库，坐席知道该改哪一句。坐席词库不提供影子屏蔽——屏蔽坐席等于客户收不到回复，那是事故不是风控。
        </Note>
      )}
