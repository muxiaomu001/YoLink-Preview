...
import { Card, PageHeader, Pill, Stat, Table } from '@/ui/display'
...
export function QuickRepliesPage() {
  ...
  const rows = useMemo(() => {
    ...
    return items.filter((q) => {
      ...
      if (!kw) return true
      return q.title.toLowerCase().includes(kw) || q.keywords.some((k) => k.toLowerCase().includes(kw)) || q.text.toLowerCase().includes(kw)
    })
  ...
  return (
    <div>
      <PageHeader
        title="话术库"
        desc="企业共享话术，员工在工作台右栏「话术」里直接发送，打字时也会按标题 / 关键词自动匹配；个人话术由员工自己在工作台维护。"
        extra={
          ...
          extra={
            <div className="flex items-center gap-2">
              <div className="relative">
                ...
                <Input value={keyword} onChange={(e) => setKeyword(e.target.value)} placeholder="搜标题 / 关键词 / 正文" className="w-56 pl-7" />
              </div>
              ...
              },
              {
                ...
                render: (q) => (
                  <div className="flex max-w-56 flex-wrap gap-1">
                    {q.keywords.length === 0 && <span className="text-zinc-300">-</span>}
                    {q.keywords.map((k) => (
                      <Pill key={k}>{k}</Pill>
                    ))}
                  </div>
