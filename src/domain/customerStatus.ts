...
export type CustomerStatusKey = 'deleted' | 'blacklisted' | 'muted' | 'mustChangePassword' | 'loggedOut'
...
/** 列表里只显示一个词时用哪个：注销 > 拉黑 > 禁言 > 待改密；都没有就是正常 */
export function primaryStatusFlag(c: Customer): CustomerStatusFlag | null {
  return customerStatusFlags(c).find((f) => f.key !== 'loggedOut') ?? null
}
...
export function matchesStatusFilter(c: Customer, filter: CustomerStatusFilter): boolean {
  ...
  // 「正常」= 一个持续性的问题状态都没有；强制下线过不算问题
  if (filter === 'normal') return flags.every((f) => f.key === 'loggedOut')
